    ...
    >
      <div className="grid grid-cols-2 gap-[8px]">
        <div className="rounded-[8px] bg-tp-slate-50 px-[8px] py-[7px]">
          <p className="text-[10px] font-medium text-tp-slate-500">{data.primaryDateLabel}</p>
          <p className="mt-[2px] text-[14px] font-bold text-tp-slate-800">{formatMoney(data.primaryRevenue)}</p>
          <p className="mt-[3px] text-[10px] text-tp-slate-500">Refunded {formatMoney(data.primaryRefunded)}</p>
          <p className="text-[10px] text-tp-slate-500">Deposits {formatMoney(data.primaryDeposits)}</p>
        </div>
        <div className="rounded-[8px] bg-tp-slate-50 px-[8px] py-[7px]">
          <p className="text-[10px] font-medium text-tp-slate-500">{data.compareDateLabel}</p>
          <p className="mt-[2px] text-[14px] font-bold text-tp-slate-800">{formatMoney(data.compareRevenue)}</p>
          <p className="mt-[3px] text-[10px] text-tp-slate-500">Refunded {formatMoney(data.compareRefunded)}</p>
          <p className="text-[10px] text-tp-slate-500">Deposits {formatMoney(data.compareDeposits)}</p>
        </div>
      </div>
      ...
      <p className="mt-[6px] text-[10px] text-tp-slate-500">{data.insight}</p>
    </CardShell>
  )
}
